...
WITH CTE_Example (Gender,Avg_Sal, Sum_salary, Max_salary,Min_salary,count_salary) AS  -- You can name column here itself, even if you aliance it lower/below it will overrider it.
(
  SELECT gender,Avg(salary),SUM(salary),MAX(salary),MIN(salary),COUNT(salary)
  FROM employee_demographics del
