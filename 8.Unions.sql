...
/* AS here we can see by employee_id,first_name, last_name merges and no duplicate is there, 
for say look at employee_id 2 it comes at last because all the remaining data is already by/in the first table
so the missing value 2 is mapped, no overlapping.*/
...
# More refined version of above UNION operation
-- by using label and ORDER BY-- 
...
-- you can put label keyword every time or just only one as it creates column only ones-- 